...
def latexify(fig_width=None, fig_height=None, columns=1):
    ...
    params = {
        # 'backend': 'ps',
        "pgf.rcfonts": False,
        # "pgf.preamble": ["\\usepackage{gensymb}", "\\usepackage[dvipsnames]{xcolor}", "\\usepackage{lmodern}"],
        "pgf.texsystem": "pdflatex",
        # "text.latex.preamble": ["\\usepackage{gensymb}", "\\usepackage[dvipsnames]{xcolor}", "\\usepackage{lmodern}"],
...
def saveimage(name, fig=plt, extension="pdf", folder="plots/", close=True):
    ...
    output_folder = os.environ.get(LEGACY_PLOTS_DIR_ENV) if folder == "plots/" else folder
